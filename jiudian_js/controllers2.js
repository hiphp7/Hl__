...
.controller('bibiCtrl', function ($scope, $state, $ionicLoading) {
	...
	$scope.gotoJiudian = function () {
		...
		$state.transitionTo('jiudian_tab.hotelhome'); //jiudian_tab.hotelhome
	};
...
.controller('hotelhomeCtrl', function ($scope, $state, $ionicModal, $ionicScrollDelegate,$timeout, Storage, publicService, $filter, MsgBox, $ionicPopover, $ionicLoading) {
	...
	$scope.gotoBibi = function () {
		$state.transitionTo('index_tab.bibi');
	};
	$scope.gotoAir = function () {
		$state.transitionTo('tab.home');
	};
	...
	//筛选条件－星级（多选）
	$scope.starArr = new Array();
	$scope.chkAll_star = true;
	//不限：true
	$scope.chkStar = function (chkAll) {
		if (chkAll) {
			$scope.chkAll_star = true;
			$scope.starArr = new Array();
			angular.forEach($scope.star_arr, function (item) {
				item['chk'] = false;
			});
		} else {
			$scope.chkAll_star = false;
		}
	};
	*/
	//搜索查询
	$scope.search = function () {
		...
		//价格区间筛选
        /*if ($scope.priceRange.value != 100) {
            $scope.jiudian_search.priceRange = (800 / 99 * $scope.priceRange.value).toFixed(0);
        } else {
            $scope.jiudian_search.priceRange = '';
        }
		*/
		...
		$scope.jiudian_Date.ruzhuDate = $filter("date")($scope.ruzhuDate, "yyyy-MM-dd");
		$scope.jiudian_Date.lidianDate = $filter("date")($scope.lidianDate, "yyyy-MM-dd");
		//星级筛选
		...
		//搜索框初始化
		$scope.jiudian_search.jiudianming = '';
		$scope.jiudian_search.starSelect = '';
        $scope.jiudian_search.priceSelect = '';
...
.controller('JiuDian_PlaceCtrl',
	...
	$http.get(ENV.imgUrl + "jiudian_data/jiudian_hot.json")
	...
	//点击‘返回’，返回首页
	$scope.back = function () {
		$state.transitionTo('jiudian_tab.hotelhome');
	};
...
.controller('HotelsCtrl', function ($scope, $state, Storage, $ionicLoading, $ionicPopover, $http, ENV, MsgBox, $timeout, $compile, $interval, $filter, publicService) {
	...
	$scope.$on('$ionicView.beforeEnter', function () {
		...
		//星级搜索条件
		//$scope.star_arr = jiudian_search.star_arr;
		//$scope.chkAll_star = jiudian_search.chkAll_star;
		$scope.starSelect = jiudian_search.starSelect;
        
		//价格区间搜索条件
		$scope.priceSelect = jiudian_search.priceSelect;
        //$scope.priceRange = jiudian_search.priceRange;
        //console.log($scope.priceRange);
		//输入框赋值
		$scope.chkAll_name = jiudian_search.jiudianming;
		//初始化输入框
		jiudian_search.jiudianming = '';
		//不同城市时，初始化加载参数
        if ($scope.cityCode !== cityCode) {
            //初始化请求数据条数和偏移量
            $scope.maxNumbel = 10;
            $scope.offset = 0;
            //第一次进入时，在进入之前，禁止触发加载更多
            $scope.noMore = true;
            //酒店列表初始化
            $scope.hotels = '';
			$scope.newhotels = '';
	...
	$scope.$on('$ionicView.enter', function () {
		//星级选中初始化
		$scope.chkStar2(true, $scope.star_arr2);
		...
		//如果查相同城市，无需再次加载数据
		var hotelList = JSON.parse(Storage.get("hotelList"));
		if (hotelList != null && hotelList[0].cityCode == $scope.cityCode) {
			...
			//搜索页星级、价格筛选
			if (Storage.get("page") === "酒店查询页") {
				//$scope.chkStar2($scope.chkAll_star, $scope.star_arr);
				$scope.hotelfilter(true);
			}
		}
		//第一次进入时，等到加载完数据后，再触发加载更多
        if ($scope.cityCode !== cityCode) {
            $scope.noMore = false;
        }
	...
	$scope.gethotelList = function () {
		...
		console.log(myobject.cityCode);
		$http.post(ENV.api + '/app/jiudian/jiudian_query/hotelList', objtops(myobject)).success(function (data) {
			if (data !== 'false') {
				...
				var hotelList = new hotelList_node($scope.cityCode, $scope.hotels, $scope.districts);
				hotelList_arr.push(hotelList);
				var hotelList_str = JSON.stringify(hotelList_arr);
				Storage.set("hotelList", hotelList_str);
	...
	//价格弹出框
	$scope.price_popover = $ionicPopover.fromTemplateUrl('price_popover.html', {
			scope: $scope
		});
	// .fromTemplateUrl() 方法
	$ionicPopover.fromTemplateUrl('price_popover.html', {
		scope: $scope
	}).then(function (popover) {
		$scope.price_popover = popover;
	});
	$scope.openPrice_popover = function ($event) {
		$scope.price_popover.show($event);
	};
	$scope.closePrice_popover = function () {
		$scope.price_popover.hide();
	};
	// 清除浮动框
	$scope.$on('$destroy', function () {
		$scope.price_popover.remove();
	});

	//星级弹出框
	$scope.star_popover = $ionicPopover.fromTemplateUrl('star_popover.html', {
			scope: $scope
		});
	// .fromTemplateUrl() 方法
	$ionicPopover.fromTemplateUrl('star_popover.html', {
		scope: $scope
	}).then(function (popover) {
		$scope.star_popover = popover;
	});
	$scope.openStar_popover = function ($event) {
		$scope.star_popover.show($event);
	};
	$scope.closeStar_popover = function () {
		$scope.star_popover.hide();
	};
	// 清除浮动框
	$scope.$on('$destroy', function () {
		$scope.star_popover.remove();
	});

	//行政区弹出框
	$scope.district_popover = $ionicPopover.fromTemplateUrl('district_popover.html', {
			scope: $scope
		});
	// .fromTemplateUrl() 方法
	$ionicPopover.fromTemplateUrl('district_popover.html', {
		scope: $scope
	}).then(function (popover) {
		$scope.district_popover = popover;
	});
	$scope.openDistrict_popover = function ($event) {
		$scope.district_popover.show($event);
	};
	$scope.closeDistrict_popover = function () {
		$scope.district_popover.hide();
	};
	// 清除浮动框
	$scope.$on('$destroy', function () {
		$scope.district_popover.remove();
	});
	// 在隐藏浮动框后执行
	$scope.$on('popover.hidden', function () {
		// 执行代码
	...
	$scope.hotelfilter = function (confirm) {
		...
		//关闭星级筛选浮动框
		$scope.closeStar_popover();

		if (confirm) {
			//搜索页传过来的，星级条件
            var arr = new Array();
            if ($scope.starSelect != '' && $scope.starSelect != null) {
                arr = arr.concat($scope.starSelect.code);
                $scope.hotelHomeStar = arr;
            } else {
                $scope.hotelHomeStar = '';
            }
		...
		$scope.filter = {
			hotelHomeStar: $scope.hotelHomeStar,
            priceSelect: $scope.priceSelect,
			star: $scope.starfilter,
            //priceRange: $scope.priceRange,
			district: $scope.districtOb
		};
		console.log('$scope.filter.priceSelect');
        console.log($scope.filter.hotelHomeStar);
		console.log($scope.filter.priceSelect);
...
.controller('HotelDetailCtrl', function ($scope, $stateParams, $ionicScrollDelegate, $timeout, $compile, $state, Storage, $filter, $interval, $ionicLoading, $http, ENV, MsgBox, publicService, $ionicSlideBoxDelegate) {
	...
	$scope.getHotelDetailList = function () {
		...
		$http.post(ENV.api + '/app/jiudian/jiudian_query/hotelDetailList', objtops(myobject)).success(function (data) {
			if (data !== 'false' && data !== '空') {
				...
				var hotelDetail = new hotelDetail_node($scope.selected_hotel.hotelId, hotelDatail_data.rooms);
				hotelDetail_arr.push(hotelDetail);
				var hotelDetail_str = JSON.stringify(hotelDetail_arr);
				Storage.set("hotelDetail", hotelDetail_str);
	...
	$scope.gotoOrder = function (room, chanpinxinxi) {
		...
		//是否需要重新加载
		if ($scope.reload) {
			MsgBox.dialogBox($compile, $scope, "酒店房型有更新，需重新查询", "确定", "getHotelDetailList()");
			return;
		}
		$scope.jiudian_selected = {
			ruzhuDate: 0,
		...
		$scope.jiudian_selected.hotelId = room.hotelId;
		$scope.jiudian_selected.roomTypeId = chanpinxinxi.roomTypeId;
...
.controller('JiuDian_TabCtrl', function($scope, $rootScope, $state,$ionicModal) {
            $rootScope.$on('$ionicView.beforeEnter', function() {
                var statename = $state.current.name;
                //tabs中存在的主页面不需要隐藏，hidetabs=false
                if (statename === 'jiudian_tab.hotelhome' || statename === 'jiudian_tab.chats' || statename === 'jiudian_tab.friends') {
...
//提交订单
    ...
    $scope.$on('$ionicView.enter', function() {
        ...
        $scope.fangjianfei = $scope.daynum * $scope.saleprice * $scope.fangjianshu.num;
        $scope.totalprice = $scope.daynum * $scope.saleprice * $scope.fangjianshu.num + $scope.expressfee;
    });
	//视图加载完成
    ...
    $scope.creatdd = function() {
        ...
        $http.post(ENV.api + '/app/jiudian/creatdingdan/validateOrder1', objtops(myobject)).success(function(data) {
            if (data !== 'false') {
                console.log(data);
                $scope.jiudian_order.orderId = data;
...
.controller('ZxfCtrl', function ($scope, $state, Storage, $http, ENV, MsgBox, $timeout, $ionicLoading) {
	...
	$scope.gotopay = function () {
		...
		//window.location.href = 'http://new.ectop.cn/hl/wxpay/jsapi_sucesss.php?ps=' + ps + '&';
		//window.location.href = 'http://new.ectop.cn/hl/wxpay/jsapi.php?ps=' + ps + '&';
		window.location.href = 'http://new.ectop.cn/hl/wxpay/jsapi_jiudian.php?ps=' + ps + '&';
		//window.location.href = 'http://new.ectop.cn/hl/wxpay/jsapi_yanzheng.php?ps=' + ps + '&';
		//        $ionicLoading.show({
		//            template: '<div><ion-spinner icon="ios"></ion-spinner><p style="color:#ddd">正在加载...</p></div>'
		//        });
		//        $http.defaults.headers.post["Content-Type"] = "application/x-www-form-urlencoded";
		...
		//        };
		//        console.log(myobject);
		//        $http.post(ENV.api + '/app/jiudian/zhifu/payOrder', objtops(myobject)).success(function(data) {
		//            //console.log(123);
		//            console.log(data);
		//            if (data != 'false') {
		//                //console.log(data1);
		//                $state.transitionTo('ddxq', {reload: true});
		//                $ionicLoading.hide();
		//            } else {
		//                alert('支付失败');
		//                $ionicLoading.hide();
		...
		//        }).error(function(error) {
		//            $ionicLoading.hide();
		//            MsgBox.promptBox("支付失败，请重新支付！", 2000);
		//            $timeout(function() {
		//                $state.transitionTo('jiudian_tab.hotelhome');
		//            }, 2000);
		//        });
...
//图片展示
        .controller('ShowimgCtrl', function($scope,Storage, $state) {
            $scope.$on('$ionicView.beforeEnter', function() {
                $scope.selected_hotel = Storage.get('selected_hotel');
            });
...
//大图
 .controller('BigimgCtrl', function($scope, $state,Storage,$stateParams,$ionicSlideBoxDelegate) {
	$scope.$on('$ionicView.beforeEnter', function() { 
		...
		$scope.selected_hotel = Storage.get('selected_hotel');
		$scope.page_count = $scope.selected_hotel.images.length;
	});
