...
class Train extends CI_Controller {
    ...
    public function trainList()
    {
        ...
        $arrStation = $this->input->get('arrStation');
        $depStation = $this->input->get('depStation');
    ...
    public function trainstatus()
    {
		...
		public function checkTicketNum() {
			...
			date_default_timezone_set("Asia/Shanghai");
			$this->load->library('yi19');
			$orderDetail = json_decode($this->input->get('orderDetail'));
			//识别座位类型
			...
			if($data->return_code == '000'){
				if(intval($data->$type) > 0 && $from_time == $data->from_time){
				   echo "余票充足";
				}else{
					echo "余票不足";
				}
