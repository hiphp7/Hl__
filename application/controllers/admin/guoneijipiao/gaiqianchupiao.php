...
class gaiqianchupiao extends CI_Controller {
    ...
    public function jiesuo($hangchengid = '') {
        ...
        redirect('admin/guoneijipiao/gaiqianchupiao/index');
    }
    ...
    public function chupiaochuli($en_id = '') {
        ...
        public function yidicp() {
            ...
            $ps = array(
                ...
                'wanchengshijian' => $chupiaoshijian,
                'chulishijian' => $chupiaoshijian,				
                'chupiaozhuangtai' => 1,
                // 订单状态 标示为已经出票
                'dingdanzhuangtai' => 3);
            ...
            $hc = $query->row();
                ...
                // 获取改签单张总费用
            $sql2 = "select n.shenqinghao as shenqinghao, n.dingdanid as fudingdanid,SUM(m.chengrenshengcangfei) AS chengrenshengcangfei,SUM(m.ertongshengcangfei) AS ertongshengcangfei,SUM(m.chengrenshouxufei) AS chengrenshouxufei,SUM(m.ertongshouxufei) AS ertongshouxufei FROM gaiqianxiangqing AS m LEFT JOIN gaiqiandingdan as n ON m.gaiqiandingdanid = n.id WHERE n.xindingdanid = ?";
            $query = $this->db->query($sql2, $dingdanid);
            $feiyong = $query->row();

            $sql3 = "SELECT SUM(CASE WHEN l.shifouertong = 0 THEN  1 ELSE   0 END ) AS daren, SUM(  CASE  WHEN l.shifouertong = 1 THEN   1 ELSE    0 END ) AS ertong FROM hangchenglvke AS l WHERE l.dingdanid = ?";

            $query = $this->db->query($sql3, $dingdanid);
            $renci = $query->row();

            $sql4 = 'select dingdanhao from dingdan where id = ?';
            $query=$this->db->query($sql4,array($feiyong->fudingdanid));
            $fudingdan = $query->row();
            $fudingdannhao = $fudingdan->dingdanhao;

            $data = new stdclass();
                $data->gaiqiandingdanhao = $feiyong->shenqinghao;  // 改签订单号
                $data->chengrenshengcangfei = $feiyong->chengrenshengcangfei;  // 成人升舱费
                $data->ertongshengcangfei = $feiyong->ertongshengcangfei;  // 儿童升舱费
                $data->fudingdanhao = $fudingdannhao;  // 父订单号
                ...
                $data->lianxiren = $dingdan->lianxiren;
                   ...
                   $data->chupiaoshijian  = $chupiaoshijian;  // 出票时间
                   $data->dingdanzhungtai  = $dingdan->dingdanzhuangtai;  // 订单状态
                   $data->dingdanzonge  = $dingdan->dingdanzonge;  // 订单总价
                   ...
                   $data->gendanrenid = $admin_session['id'];  // 跟单人ID
                   ...
                   $this->db->insert('caiwu_fj_gaiqian' , $data);
		// 财务详情
        // $dingdanid = 213;
            ...
            if ($this->db->trans_status() === FALSE) {
                ...
                if ($dingdanid != 0) {
					...
					if(!empty($fasong->airline_fan)){
					  $this->myalidayu->GaiQianChengGong_wangfan_v5($fasong->tel, $fasong->name, $fasong->date, $fasong->dep, $fasong->arr, $fasong->airline, $fasong->flight,$fasong->deptime, $fasong->date_fan, $fasong->dep_fan, $fasong->arr_fan, $fasong->airline_fan, $fasong->flight_fan,$fasong->deptime_fan);                  
					} else {
					  $this->myalidayu->GaiQianChengGong_v5($fasong->tel, $fasong->name, $fasong->date, $fasong->dep, $fasong->arr, $fasong->airline, $fasong->flight,  $fasong->deptime);
        ...
        public function bendicp() {
            ...
            $hc = $query->row();
                ...
                // 获取改签单张总费用
            $sql2 = "select n.shenqinghao as shenqinghao, n.zhudingdanid as zhudingdanid,SUM(m.chengrenshengcangfei) AS chengrenshengcangfei,SUM(m.ertongshengcangfei) AS ertongshengcangfei,SUM(m.chengrenshouxufei) AS chengrenshouxufei,SUM(m.ertongshouxufei) AS ertongshouxufei FROM gaiqianxiangqing AS m LEFT JOIN gaiqiandingdan as n ON m.gaiqiandingdanid = n.id WHERE n.xindingdanid = ?";
            $query = $this->db->query($sql2, $dingdanid);
            $feiyong = $query->row();
            ...
            $sql4 = 'select dingdanhao from dingdan where id = ?';
            $query=$this->db->query($sql4,array($feiyong->zhudingdanid));
            $zhudingdan = $query->row();
            $zhudingdanhao = $zhudingdan->dingdanhao;

            $data = new stdclass();
                $data->gaiqiandingdanhao = $feiyong->shenqinghao;  // 改签订单号
                $data->chengrenshengcangfei = $feiyong->chengrenshengcangfei;  // 成人升舱费
                $data->ertongshengcangfei = $feiyong->ertongshengcangfei;  // 儿童升舱费
                $data->fudingdanhao = $zhudingdanhao;  // 父订单号
                $data->chupiaodingdanhao = $dingdan->dingdanhao ;  // 出票订单号
                $data->lianxiren = $dingdan->lianxiren;
                   $data->lianxirendianhua = $dingdan->lianxirendianhua;  // 联系人电话
                   $data->chuangjianshijian  = $dingdan->chuangjianshijian;  // 创建时间
