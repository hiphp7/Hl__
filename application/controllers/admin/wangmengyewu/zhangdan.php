...
class zhangdan extends CI_Controller {
    ...
    public function jiudian()
    {
        ...
        // $zhangdanhao = "wm".date("YmdHis").rand(1000,9999); // 账单号
        $shanghuhao = 10000;
        $start_time = "2017-3-1 0:0:0";
        $end_time = "2017-6-1 0:0:0";
   
        $sql_jiudian = 'select m.affiliateConfirmationId as piaoshu,m.numberOfRooms as numberOfRooms ,m.numberOfCustomers as numberOfCustomers from jiudian_chuangjiandingdan as m WHERE m.dingdanzhuangtai = 2  and m.shanghuhao = ? and  m.fukuanshijian BETWEEN ? AND ?';
        $query = $this->db->query($sql_jiudian, array($shanghuhao, $start_time, $end_time));
        $row = $query->result();
        $jianpiaoall = 0;
        $dingdanall = 0;
    ...
    public function savezhangdan_test() {
        $zhangdanhao = "wm".date("YmdHis").rand(1000,9999); // 账单号
        $shanghuhao = 10000;
        $start_time = "2017-3-1 0:0:0";
        $end_time = "2017-6-1 0:0:0";
                            ...
                            if ($v1->qianyuefangshi == 0) {
                                ...
                                if ($v1->qianyuefangshi == 0) {
                                    $baoxian = array(
                                        ...
                                        "tuihuoshu" => 0,
                                        "yongjinjishu" => $yongjinjishu,
                                        "youxiaoshu" => $chubaoxiancount,
                                        "qianyuefangshi" => 0,
                                        "yongjine" => $chubaoxiancount * $yongjinjishu
                                    );
                                    $this->db->insert('wm_zhangdanmingxi', $baoxian);
                                    $yongjinzonge += $chubaoxiancount * $yongjinjishu;
