...
class Trainorder extends CI_Controller {
    ...
    public function save() {
		...
		if($query->num_rows() > 0){
			...
			$data->seat_type = $obj->seatList[0]->typeId;
    ...
    public function create19yiorder() {
		...
		if(empty($fukuanshijian)){
			...
			$data = $this->sendmessage->send('@all','你有一个新的<火车票>订单');
			$myfile = "sendmessage.txt";
			$txt = $data.'--订单号:'.$merchant_order_id.'付款时间:'.$fukuanshijianall;
...
/*						
	...
	*/	
	...
	public function trainorderlist()
	{
		$this->db->trans_begin();
		$yonghuid = $this->input->post('yonghuid');
		$shaixuan = $this->input->post('status');
	...
	public function orderdetail()
	{
		...
		if (!empty($result) && $result != false) {
            ...
            if (strtotime($result->from_time) <= time()) {
                $result->ketuipiao = false;
            } else {
	...
	public function ordertuipiao() {
		$this->db->trans_begin();
		$this->load->library('yi19');
		$str = json_decode($this->input->post('tuipiao'));
		...
		$tdingdan->shanghuhao = $shanghuhao;
		$tdingdan->chuangjianshijian = date("Y-m-d H:i:s");
	...
	function aaa(){
		$this->db->update('train_order_lvke', array('ticketStatusName' => '已退票'), array('id' => '65'));
	}
