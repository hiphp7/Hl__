...
class flight extends CI_Controller {
    ...
    public function myquery() {
        ...
        // 获取参数
        $depCity = $this->input->post('depCity');
        $arrCity = $this->input->post('arrCity');
        //注：date第一次为去程时间；若有返程，点击选返程，则第二次为返程时间。
        $date = $this->input->post('date');
        $mytrip = $this->input->post('mytrip');
        ...
        $sql = "select shangbanshijian, xiabanshijian, zhouqi  from peizhi where chanpinleixing = '飞机' and yewuleixing = '出票'";
        ...
        if (strval($xml->returnCode) == "S") {
            ...
            foreach ($flights as $f) {
                ...
                //$day = date("Ymd", strtotime($f->arrTime));
                $today = date("Y-m-d");
                $todaytime = date("Hi");
                ...
                $mintime = date('Hi', strtotime('+6 hour'));

                $chufatime = date("Hi", strtotime($f->arriTime));
                if ($date == $today) {
                    //var_dump(1);
                    if (intval($todaytime) < 1800) {
                        //	var_dump(2);
                        if (intval($mintime) > intval($chufatime)) {
                            //		var_dump(3);
                            continue;
                        }
                    } else {
                ...
                //	if($chufatime < $shangbanshijian && $chufatime > $xiabanshijian){
                //		continue;
                //	}
                ...
                foreach ($f->seatItems as $s) {
                    ...
                    $s->piaomianjia = intval(strval($s->parPrice)); // 票面价
