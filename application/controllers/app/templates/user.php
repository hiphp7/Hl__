...
class user extends CI_Controller {
    ...
    public function FindDengLuMing() {
        // number可以是手机号码或邮箱号码
        $ps = $this->input->post('number');
        $mima = $this->input->post('mima');
        ...
        $sql = 'select m.id as id, m.shanghuhao as shanghuhao, m.zhanghu as zhanghu, m.mima as mima, m.xingming as xingming, m.shoujihaoma as shoujihaoma from yonghu as m where m.mima = ? and ';
...
m.chengshiip as chengshiip from yonghu as m where m.id = ?';
    ...
    public function upload() {
		...
		$new_file = "/usr/local/apache/htdocs/hl/resources/user/userImages/" . $UserId . ".{$type}";
        $base64_decode = base64_decode($base64);
        if (write_file($new_file, $base64_decode)) {
			$this->db->update('yonghu', array('img' => $UserId . ".{$type}"), array('id' => $UserId));
            echo $UserId . ".{$type}";
        } else {
            echo false;
        }
    ...
    public function AddZhuCeRiQi() {
        ...
        if (!empty($sf) && strlen($sf) == 6) {
            ...
            $query = $this->db->query($biaoshi, array($str));
            $res = $query->row();
            if(!empty($res)){
        ...
        $sql = 'select m.id as id from yonghu as m where m.zhanghu = ? ';
        $res = $this->db->query($sql, $this->input->post('shoujihaoma'));
        $row = $res->first_row();
        if (!empty($row) && intval($row->id) > 0) {
            echo 'ok';
        } else {
            $obj = array(
                'zhanghu' => $this->input->post('shoujihaoma'),
                'shoujihaoma' => $this->input->post('shoujihaoma'),
                'mima' => $this->input->post('mima'),
     ...
     *  验证手机短信
     */
    public function shoujiyanzhengma()
    {
        $this->load->library('session');
        $YanZhengMa = $this->input->post('ymz');
        $ymz = $this->session->set_userdata('yzm');
        if ($YanZhengMa == $ymz) {
    ...
    public function updateUserData() {
        ...
        $UserId = $this->input->post('UserId');
        // 用户姓名
        $xingming = $this->input->post('xingming');
        // 用户性别
        $xingbie = $this->input->post('xingbie');
        // 用户密码
        $mima = $this->input->post('mima');
        // 用户手机号码
        $zhanghu = $this->input->post('zhanghu');
        // 用户邮箱
    ...
    public function YanZhengMa() {
        ...
        $result = $this->myalidayu->YanZhengMa($yzm, '', $shoujihaoma);
    ...
    public function orderList() {
		...
		$gaiqi_show  = true;
        $tuipiao_show  = true;
        $sql = "select shangbanshijian, xiabanshijian, zhouqi  from peizhi where chanpinleixing = '飞机' and yewuleixing = '改期'";
        $query = $this->db->query($sql);
        $data= $query->row();
        $shangbanshijian = $data->shangbanshijian;
        $xiabanshijian = $data->xiabanshijian;
        $zhouqi = $data->zhouqi;
        $zhouqi = explode(",", $zhouqi);
        $todayweak = date("w");
        $todaytime = date("Hi");
        if(intval($todaytime) < intval($shangbanshijian) || intval($todaytime) > intval($xiabanshijian)){
            $gaiqi_show  = false;
        }

        if (! in_array($todayweak, $zhouqi)) {
            $gaiqi_show  = false;
        }

        $sql = "select shangbanshijian, xiabanshijian, zhouqi  from peizhi where chanpinleixing = '飞机' and yewuleixing = '退票'";
        $query = $this->db->query($sql);
        $data= $query->row();
        $shangbanshijian = $data->shangbanshijian;
        $xiabanshijian = $data->xiabanshijian;
...
m.istaitou as istaitou,
m.qiyeshuihao as qiyeshuihao,
m.shanghuhao as shanghuhao,
m.fapiaomingzi as fapiaomingzi,
m.fapiaoleixing as fapiaoleixing,
    ...
    $orders_res = $this->db->query($orders_sql, array($UserId));
        ...
        if (!empty($orders)) {
            ...
            foreach ($orders as $od) {
				...
				$myorder->shanghuhao = $od->shanghuhao;
                //付款时间
				...
				if($od->isgaiqian == "1" && $od->dingdanzhuangtai == "3"){
                     $myorder->statusName = "改签完成";
                }
                //出票状态
				...
				$myorder->isgaiqian = $od->isgaiqian;
                //邮寄人电话
                $myorder->youjirendianhua = $od->youjirendianhua;
				
				$myorder->lianxiren = $od->lianxiren;
				$myorder->lianxirendianhua = $od->lianxirendianhua;
                $myorder->fapiaoleixing = $od->fapiaoleixing;         
                $myorder->fapiaomingzi = $od->fapiaomingzi;         
                $myorder->qiyeshuihao = $od->qiyeshuihao;         
                $myorder->istaitou = $od->istaitou;				
                //获取订单表数据 [end]
...
m.beizhu as beizhu from baoxiandingdan as m where m.dingdanid = ?';
                ...
                $myorder->gaiqi_show = $gaiqi_show;
                $myorder->tuipiao_show = $tuipiao_show;				
                $orders_ss[] = $myorder;
            }
            $orderlist = array();

            foreach ($orders_ss as $v) {
				$chuangjianshijianchuo = $v->chuangjianshijianchuo;
				$timer=strtotime('now');
				$zhongzhishijianchuo=$chuangjianshijianchuo+10*60; // 支付最后时间
			    if ($v->dingdanzhuangtai == '0' && $zhongzhishijianchuo < $timer) {

					$this->db->update('dingdan', array('dingdanzhuangtai' => 6), array('id' => $v->orderid));
					$v->dingdanzhuangtai == '6';
					$v->statusName == '订单超时';
					if($v->isgaiqian == '1'){
						$sql = 'select m.id as id, m.shenqinghao as shenqinghao from gaiqiandingdan as m where m.xindingdanid = ?';
						$query = $this->db->query($sql, $v->orderid);
						$res = $query->row();
						$shenqinghao = $res->shenqinghao;       
						$sql2 = 'select m.id as id, m.hangchenglvkeid as hangchenglvkeid from gaiqian as m where m.shenqinghao = ?';
						$query = $this->db->query($sql2, $shenqinghao);
						$re = $query->result();
						foreach ($re as $v) {          
							$this->db->update('hangchenglvke',array('zhuangtai'=>'已出票'),array('id'=>$v->hangchenglvkeid));
						}	
					}
				} 
                if ($status == '1') {
   ...
   // 提交改签数据
	public function alterRequest() {
		date_default_timezone_set("Asia/Shanghai");
		$this->db->trans_begin();
		$gaiqianxinxi = json_decode($this->input->post('submitInfo'));
		$dingdanid = '';
        ...
        $sql = 'select m.zhudingdanid as zhudingdanid from gaiqiandingdan as m where m.xindingdanid = ?';
        $query = $this->db->query($sql,array($dingdanid));
        $zhudingdan = $query->row();
        if (empty($zhudingdan)) {
                    
            $zhudingdanid = $dingdanid;
        } else {
            $zhudingdanid = $zhudingdan->zhudingdanid;
        }


        $this->db->insert('gaiqiandingdan', array('zhudingdanid'=>$zhudingdanid,'dingdanid'=>$dingdanid,'chulizhuangtai'=>0, 'shenqingzhuangtai'=>0, 'shenqinghao' => $shengqinghao,'shenqingriqi' =>date("Y-m-d H:i:s")));
		...
		if ($this->db->trans_status() === FALSE) {
			...
			$data = $this->sendmessage->send('@all','你有一个新的<国内机票-改签>订单，请及时处理');
			$myfile = "sendmessage.txt";
			$txt = $data.'--飞机改签申请号:'.$shengqinghao.'时间:'.date("Y-m-d H:i:s");
	...
	public function refundRequest() {
		date_default_timezone_set("Asia/Shanghai");
		$this->db->trans_begin();
		$tuipiaoxinxi = json_decode($this->input->post('refundInfo'));
		$dingdanid = '';
		...
		$shanghuhao = $tuipiaoxinxi->shanghuhao;
		// 去程
        if ($tuipiaoxinxi->goto->chk == true) {
            ...
            $res = $query->row();
            if (empty($res)) {
                $this->db->insert('tuipiaodingdan', array('dingdanid'=>$dingdanid,'shanghuhao'=>$shanghuhao,'tuipiaoleixing'=>$tuipiaoleixing, 'chulizhuangtai'=>0));
            }
		...
		if ($this->db->trans_status() === FALSE) {
			...
			$this->db->trans_commit();
			$this->load->library('sendmessage');	
			$data = $this->sendmessage->send('@all','你有一个新的<国内机票-退票>订单，请及时处理');
			$myfile = "sendmessage.txt";
			$txt = $data.'--飞机退票订单id:'.$dingdanid.'时间:'.date("Y-m-d H:i:s");
			file_put_contents($myfile, $txt, FILE_APPEND);
			$txt = "\r\n";
			file_put_contents($myfile, $txt, FILE_APPEND);			
			echo 'true';
...
m.beizhu as beizhu from lvke as m where m.yonghuid = ?';
    ...
    public function addLvKe() {
        ...
        //转换为证件类型的数字表示
        foreach ($zjlx as $k => $v) {
            if ($this->input->post('zhengjianleixing') == $v) {
                $zhengjianleixing = $k;
            }
        }
        $yonghuid = intval($this->input->post('UserId'));
        ...
        $chushengriqi = $this->input->post('chushengriqi');
		$shifouertong = $this->input->post('shifouertong');
        $zhengjianhaoma = $this->input->post('zhengjianhaoma');
        $shoujihao = $this->input->post('shoujihao');
        $obj = array(
            'yonghuid' => $yonghuid,
...
m.beizhu as beizhu from yonghudizhi as m where m.yonghuid = ?';
    ...
    public function addYongHuDiZhi() {
        date_default_timezone_set("Asia/Shanghai");
        $yonghuid = intval($this->input->post('UserId'));
        $shoujianrenmingzi = $this->input->post('shoujianrenmingzi');
        $shoujihao = $this->input->post('shoujihao');
        $dizhi = $this->input->post('dizhi');
        $youbian = $this->input->post('youbian');
        $obj = array(
            'yonghuid' => $yonghuid,
