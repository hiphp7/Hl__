...
class jiudian_query extends CI_Controller {
	...
	*/
	...
	public function hotelSearch() {
        ...
        $this->db->trans_begin();
        $cityName = $this->input->get('cityName')."市";
		//微信定位
		$position_name = $this->input->get('position_name');
        //分页
        $offset = $this->input->get('offset') + 1;
        $searchBox = $this->input->get('searchBox');
        $starSelect = $this->input->get('starSelect');
        $priceSelect = $this->input->get('priceSelect');
        if(empty($searchBox)){
...
m.value as value from jiudian_locations as m where m.imageid = ? LIMIT 1 OFFSET 0';
    ...
    public function hotelDetailList() {
        ...
        $ruzhuDate = $this->input->get('ruzhuDate');
        $lidianDate = $this->input->get('lidianDate');
