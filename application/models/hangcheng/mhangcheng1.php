...
class mhangcheng extends CI_Model {
        ...
        if ($id != 0) {
            ...
            if ($res->num_rows() > 0) {
                foreach ($res->result() as $r) {
                    ...
                    if(!empty($r->ranyoushui))
                    {
                        $obj->jijianfei = $r->ranyoushui;
                    }
                    else
                    {
                        $obj->jijianfei = '';
    ...
    public function getSaveObj($selected, $dingdan_obj, $dingdanid, $lvke, $baoxian, $costdetail) {
        ...
        if ($selected->goto->date != '') {
            ...
            // 票面单价
            $hangcheng->piaomiandanjia = $selected->goto->seatItem->parPrice;
            ...
            $hangcheng->ranyoushui = $sub_airportTax;
            ...
            foreach ($lvke as $v) {
                    ...
                    if ($v->shifouertong == 0) {
						$hclc->seatCode = $hangcheng->seatCode;
						$hclc->piaomianjia = $costdetail->goto->adult->unitprice; // 票面价
        ...
        if ($selected->back->date != '') {
            ...
            // 票面单价
            $hangcheng->piaomiandanjia = $selected->back->seatItem->parPrice;
            ...
            foreach ($lvke as $v) {
                    ...
                    if ($v->shifouertong == 0) {
						$hclc->seatCode = $hangcheng->seatCode;
                        $hclc->piaomianjia = $costdetail->back->adult->unitprice; // 票面价
