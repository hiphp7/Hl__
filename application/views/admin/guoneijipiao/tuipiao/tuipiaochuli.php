<!-- 弹出框 -- 占座 -->
<div aria-labelledby="win_zhanzuo" role="dialog" tabindex="-1" class="modal fade bs-example-modal-sm" id="win_zhanzuo" style="display:none;">
    <div role="document" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-body">
                <div class="container-fluid">
            <!-- 退改签说明： -->
            <div style="margin-left: 5px;">
                <div style="margin-left: 20px;">
                   <div style="line-height: 30px;">
                      <div >
                          <div style="width: 15%; float: left;">退票费</div>
                          <div style="width: 80%; float: left;" id="a_refund1" >
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞<span class="a_refundTimePointAadvance"></span>小时之前 <span style="right: 20px; float: right;"><span class="a_refundPercentAdvance"></span>%</span></div>

                              </div>
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞<span class="a_refundTimePointAadvance"></span>小时到<span class="a_refundTimePoint"></span>小时之前<span style="right: 20px; float: right;"><span class="a_refundPercentBefore"></span>%</span></div>

                              </div>
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞<span class="a_refundTimePoint"></span>小时内及飞后<span style="right: 20px; float: right;"><span class="a_refundPercentAfter"></span>%</span></div>
                              </div>                                
                          </div>
                          <div style="width: 80%; float: left;" id="a_refund2">
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞<span class="a_refundTimePoint"></span>小时之前 <span style="right: 20px; float: right;"><span class="a_refundPercentBefore"></span>%</span></div>
                              </div>
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞<span class="a_refundTimePoint"></span>小时内及飞后<span style="right: 20px; float: right;"><span class="a_refundPercentAfter"></span>%</span></div>
                              </div>
                          </div>

                          <div style="width: 80%; float: left;" id="a_refund3">
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞前 <span style="right: 20px; float: right;"><span class="a_refundPercentBefore"></span>%</span></span></div>
                              </div>
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞后<span style="right: 20px; float: right;"><span class="a_refundPercentBefore"></span>%</span></div>
                              </div>
                          </div>
                          <div style="width: 80%; float: left;" id="a_refund4">
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞前 <span style="right: 20px; float: right;">免费</span></div>
                              </div>
                              <div style="width: 100%; float: left;">
                                  <div style="margin-right:10px;">起飞后<span style="right: 20px; float: right;">免费</span></div>
                              </div>
                          </div>     
                      </div>
                      </div>
                    <div style="clear: both;"></div>
                     <div>
                        <div style="width: 15%; float: left;">改期费</div>
                        <div style="width: 80%; float: left;" id="a_change1">
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞<span class="a_changeTimePointAdvance"></span>小时之前 <span style="right: 20px; float: right;"><span class="a_changePercentAdvance"></span>%</span></div>

                            </div>
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞<span class="a_changeTimePointAdvance"></span>小时到<span class="a_changeTimePoint"></span>小时之前<span style="right: 20px; float: right;"><span class="a_changePercentBefore"></span>%</span></div>

                            </div>
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞<span class="a_changeTimePoint"></span>小时内及飞后<span style="right: 20px; float: right;"><span class="a_changePercentAfter"></span>%</span></div>
                            </div>                                
                        </div>
                        <div style="width: 80%; float: left;" id="a_change2">
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞<span class="a_changeTimePoint"></span>小时之前 <span style="right: 20px; float: right;"><span class="a_changePercentBefore"></span>%</span></div>
                            </div>
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞<span class="a_changeTimePoint"></span>小时内及飞后<span style="right: 20px; float: right;"><span class="a_changePercentAfter"></span>%</span></div>
                            </div>
                        </div>

                        <div style="width: 80%; float: left;" id="a_change3">
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞前 <span style="right: 20px; float: right;"><span class="a_changePercentABefore"></span>%</span></span></div>
                            </div>
                            <div style="width: 100%; float: left;">
                                <div style="margin-right:10px;">起飞后<span style="right: 20px; float: right;"><span class="a_changePercentAfter"></span>%</span></div>
                            </div>                            
                        </div>
                        <div style="width: 80%; float: left;" id="a_change4">
                          <div style="width: 100%; float: left;">
                              <div style="margin-right:10px;">起飞前 <span style="right: 20px; float: right;">免费</span></div>
                          </div>
                          <div style="width: 100%; float: left;">
                              <div style="margin-right:10px;">起飞后<span style="right: 20px; float: right;">免费</span></div>
                          </div>                            
                      </div>

                  </div>                   
                </div>
                </div>
                </div>
            </div>
            <div class="modal-footer">
                <button data-dismiss="modal" class="btn btn-default" type="button">关闭</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div> 
<!-- 弹出框 -- 查看 -->
<div aria-labelledby="win_yidi" role="dialog" tabindex="-1" class="modal fade bs-example-modal-sm" id="win_yidi" style="display:none;">
    <div role="document" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            </div>
            <div class="modal-body">
                <div class="container-fluid">
                      <table class="table">
                            <?php if (!empty($obj) && !empty($obj->yidipingtai) && $obj->yidipingtai != ''): ?>
                            <tbody>
                                <tr>
                                    <td>异地平台名称:</td>
                                    <td><?php echo $obj->yidipingtai; ?></td>
                                </tr>
                                <tr>
                                    <td>采购金额:</td>
                                    <td><?php echo $obj->yidicaigoujine; ?>人民币（元）</td>
                                </tr>
                                <tr>
                                    <td>异地采购单号:</td>
                                    <td><?php echo $obj->yididingdanhao; ?></td>
                                </tr>
                                <tr>
                                    <td>其他说明:</td>
                                    <td><?php echo $obj->yidiqitashuoming; ?></td>
                                </tr>
                            </tbody>
                            <?php endif; ?>
                        </table>
                </div>
            </div>
            <div class="modal-footer">
                <button data-dismiss="modal" class="btn btn-default" type="button">关闭</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>

<!-- 弹出框 -- 查看 -->
<div aria-labelledby="win_bendi" role="dialog" tabindex="-1" class="modal fade bs-example-modal-sm" id="win_bendi" style="display:none;">
    <div role="document" class="modal-dialog">
        <div class="modal-content">
            <div class="modal-header">
            </div>
            <div class="modal-body">
                <div class="container-fluid">
                      <table class="table">
                            <?php if (!empty($obj) && !empty($obj->prn) && $obj->prn != ''): ?>
                            <tbody>
                                <tr>
                                    <td>订座记录PNR:</td>
                                    <td><?php echo $obj->prn; ?></td>
                                </tr>
                                <tr>
                                    <td>采购金额:</td>
                                    <td><?php echo $obj->caigoujine; ?>人民币（元）</td>
                                </tr>
                                <tr>
                                    <td>其他说明:</td>
                                    <td><?php echo $obj->qitashuoming; ?></td>
                                </tr>
                            </tbody>
                            <?php endif; ?>
                        </table>
                </div>
            </div>
            <div class="modal-footer">
                <button data-dismiss="modal" class="btn btn-default" type="button">关闭</button>
            </div>
        </div><!-- /.modal-content -->
    </div><!-- /.modal-dialog -->
</div>
           <div aria-labelledby="mymodalLabel2" role="dialog" tabindex="-1" class="modal fade in"
                    id="waicaidingdanxk" style="display: none;">
                    <div role="document" class="modal-dialog" style="width:300px;">
                        <div class="modal-content">
                            <form id='frm_gaoji'>
                            <div class="modal-header">
                                <button aria-label="Close" data-dismiss="modal" class="close" type="button">
                                    <span aria-hidden="true">×</span></button>
                                <h4 id="mymodalLabel2" class="modal-title">
                                    外采信息</h4>
                            </div>
                            <div class="modal-body">
                                <div class="container-fluid">
                                    <div class="row">
                                        <table id="prohighsearchtable" style="width: 100%;">
                                            <tbody>
                                                <tr>
                                                    <td style="font-size: medium; font-weight: bolder;">
                                                        外采平台
                                                    </td>
                                                    <td>
                                                        <input type="text" placeholder="外采平台" id="chupiaobianma" />
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td style="font-size: medium; font-weight: bolder;">
                                                        外采订单编号
                                                    </td>
                                                    <td>
                                                        <input type="text" placeholder="外采订单编号" id="dingdanhao" />
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td style="font-size: medium; font-weight: bolder;">
                                                        成人票单价
                                                    </td>
                                                    <td>
                                                        <input type="text" placeholder="成人票单价" id="zhongwenming" />
                                                    </td>
                                                </tr>
                                                <tr>
                                                    <td style="font-size: medium; font-weight: bolder;">
                                                        备注说明
                                                    </td>
                                                    <td>
                                                        <input type="text" placeholder="备注说明" id="beizhu" />
                                                    </td>
                                                </tr>
                                            </tbody>
                                        </table>
                                    </div>
                                </div>
                            </div>
                            <div class="modal-footer">
                                <button class="btn btn-primary" id="btn_reseach" type="button">
                                    关闭</button>
                            </div>
                            </form>
                        </div>
                    </div>
                </div>
			<div class="row">
                    <div class="col-md-12">
                        <h2 style="font-size: 30px; margin-bottom: 0px;">
                            退票订单详情</h2>
                    </div>
                    <div class="col-md-12">
                        <h4>
                            <a href='<?php echo site_url("admin/guoneijipiao/chupiao/index"); ?>'>机票订单管理</a> / <a href='<?php echo site_url("admin/guoneijipiao/chupiao/index"); ?>'>退票管理</a> / <a href='#'>退票处理</a></h4>
                    </div>
                </div>
               <?php $CI = & get_instance(); echo form_open('admin/guoneijipiao/tuipiao/tp', array('id' => 'tuipiaoform', 'name' => 'tuipiaoform', 'class' => 'form-signin'));?>
                <!-- 退票成功 -->
                <div class="row">
                    <div class="col-md-12">
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <span style="font-size:18px;"><?php echo $obj->dingdanhao; ?>&nbsp;&nbsp;</span><span style="font-size:18px; color:Red;"><?php echo $obj->chulizhunagtai; ?></span>
                            </div>
                            <div class="panel-body">
                                <table class="table" style="margin-bottom:0px;">
                                     <tbody>
                                          <tr>
                                              <td>订单来源：<?php echo $obj->dingdanlaiyuan; ?></td>
                                              <!--
                                              <td>创建时间：2016-04-12 12:30:55</td>
                                              -->
                                              <td>支付时间：<?php echo $obj->fukuanshijian; ?></td>
                                              <td>申请退票时间：<?php echo $obj->chulishijian; ?></td>
                                          </tr>
                                          <tr>
                                              <td>支付渠道：<?php echo $obj->zhifufangshi; ?></td>
											<td>订单总价：￥<?php
												if ($obj->isgaiqian == 1) {
													echo $obj->dingdanzonge;
												} else {
													$CI = & get_instance();
													$sql = 'select count(m.id) * m.dingdanzongjia  as baoxianzongjia from baoxiandingdan as m where m.dingdanid = ?';
													$query = $CI->db->query($sql, $obj->id);
													$res = $query->row();
													echo $obj->dingdanzonge - $res->baoxianzongjia;
												};
												?></td>

                                              <td></td>
                                          </tr>
                                     </tbody>
                                </table>
                            </div>
                        </div>
                    </div>
                </div>
                <!-- 详细 -->
                <div class="row">
                    <div class="col-md-12">
                        <?php 
                        $index = 1;
						if(!empty($hangcheng))
						{
                        foreach ($hangcheng as $hc): ?>
                        <div class="panel panel-primary">
                            <div class="panel-heading">
                                <span style="font-size:18px;">第 <?php echo $index; ?> 段</span>
                            </div>
                            <div class="panel-body">
                                <table class="table mytable" style="margin-bottom:0px;">
                                     <thead>
                                          <tr>
                                              <th>航程类型</th>
                                              <th>航空公司</th>
                                              <th>航班号</th>
                                              <th>起飞/到达</th>
                                              <th>经停</th>
                                              <?php if (!empty($obj) && !empty($obj->yidipingtai) && $obj->yidipingtai != ''): ?>
                                              <th>异地退票信息</th>
                                              <?php else: ?>
                                              <th>本地BSP打票</th>
                                              <?php endif; ?>
                                          </tr>
                                     </thead>
                                     <tbody>
                                          <tr>
										      <?php if ($hc->wangfanhangcheng == 0): ?>
                                              <td>单程</td>
											  <?php else: ?>
											  <td>往返</td>
											  <?php endif; ?>
                                              <td><?php echo $hc->hangkonggongsi; ?></td>
                                              <td><?php echo $hc->hangbanhao; ?></td>
											  <!-- 
                                              <td>北京首都机场T2 - 上海浦东机场T1  2016-12-12  23:00 - 00:30 +1天</td>
											  -->
											  <td><?php echo $hc->qifeijichang.$hc->qifeihangzhanlou.' - '.$hc->daodajichang.$hc->daodahangzhanlou.' '.$hc->qifeishijian.' '.$hc->daodashijian; ?></td>
											  <?php if ($hc->jingtinglianbiao > 0): ?>
                                                                                          <td>是</td>
											  <?php else: ?>
											  <td>否</td>
											  <?php endif; ?>
                                                                                          <?php if (!empty($obj) && !empty($obj->yidipingtai) && $obj->yidipingtai != ''): ?>
                                                                                          <td><button type="button" class="btn btn-info" id='chakan_a'>查看</button></td>
                                                                                          <?php else: ?>
                                                                                          <td><button type="button" class="btn btn-info" id='chakan_a_bendi'>查看</button></td>
                                                                                          <?php endif; ?>
                                          </tr>
                                     </tbody>
                                </table>
                                <br />
                                <table class="table mytable" style="margin-bottom:0px;">
                                     <thead>
                                          <tr>
                                              <th>乘客类型</th>
                                              <th>乘客数量</th>
                                              <th>舱位</th>
                                              <th>购买价/张</th>
                                              <th>票面价/张</th>
                                              <th>民航基金/张</th>
                                              <th>退票费/张</th>
                                              <th>实际退款/张</th>
                                              <th>出票编码</th>
                                              <th>退改签规则</th>
                                          </tr>
                                     </thead>
                                     <tbody>	
                                         <?php if (!empty($tuipiao) && !empty($tuipiao->hc) && $index == 1): ?>
                                         <?php foreach ($tuipiao->hc as $c): 
                                             // 表示是否是儿童
                                             $myet = 1;
                                             $rt = 1;
                                             if($c->shifouertong == '成人')
                                             {
                                                 $myet = 0;
                                                 $rt = 0;
                                             }
                                             
                                             $rt .= '_'.$index;
                                             ?>
                                          <tr>
                                              <td><?php echo $c->shifouertong; ?></td>
                                              <td><?php echo $c->renshu; ?>人</td>
                                              <td><?php echo $c->cangwei; ?></td>
                                              <td>&yen;<span id='gmjg_<?php echo $rt; ?>'><?php echo floatval($c->piaomiandanjia); ?></span>
                                                  <input type="hidden" id='total_fee<?php echo $rt; ?>' name='total_fee<?php echo $rt; ?>' value='<?php echo floatval($c->piaomiandanjia) + floatval($c->jijianfei); ?>'/>
                                              </td>
                                              <td>&yen;<?php echo $c->piaomianjia; ?></td>

                                              <td>&yen;<span id='jjf_<?php echo $rt; ?>'><?php echo floatval($c->jijianfei); ?></span></td>
                                              <td><input id='chupiaobianma_<?php echo $rt; ?>' hangchengid="<?php echo $c->hangchengid; ?>" ertong ="<?php echo $myet; ?>" fancheng="<?php echo $index; ?>" onkeyup='jishuan("gmjg_<?php echo $rt; ?>",  "jjf_<?php echo $rt; ?>","chupiaobianma_<?php echo $rt; ?>", "sjtk_<?php echo $rt; ?>", "<?php echo $rt; ?>")' name='chupiaobianma_<?php echo $rt; ?>' type="text" placeholder="退票费/张"/></td>
                                              <td><span id='sjtk_<?php echo $rt; ?>'></span><input type="hidden" hangchengid="<?php echo $c->hangchengid; ?>" renshu="<?php echo $c->renshu; ?>" ertong ="<?php echo $myet; ?>" fancheng="<?php echo $index; ?>" id='refund_fee<?php echo $rt; ?>' name='refund_fee<?php echo $rt; ?>'/></td>
                                              <td><?php echo $c->chupiaobianma; ?></td>
                                              <td><button type="button" class="btn btn-warning" onclick="tuigaiqianguize('<?php echo $c->tuigaiid; ?>');">退改签规则</button></td>
                                          </tr>
                                          <?php endforeach; ?>
                                          <?php endif; ?>
                                          
                                          <?php if (!empty($tuipiao1) && !empty($tuipiao1->hc) && $index == 2): ?>
                                         <?php foreach ($tuipiao1->hc as $c): 
                                             // 表示是否是儿童
                                             $myet = 1;
                                             $rt = 1;
                                             if($c->shifouertong == '成人')
                                             {
                                                 $myet = 0;
                                                 $rt = 0;
                                             }
                                             
                                             $rt .= '_'.$index;
                                             ?>
                                          <tr>
                                              <td><?php echo $c->shifouertong; ?></td>
                                              <td><?php echo $c->renshu; ?>人</td>
                                              <td><?php echo $c->cangwei; ?></td>
                                              <td>&yen;<span id='gmjg_<?php echo $rt; ?>'><?php echo floatval($c->piaomiandanjia); ?></span>
                                                  <input type="hidden" id='total_fee<?php echo $rt; ?>' name='total_fee<?php echo $rt; ?>' value='<?php echo floatval($c->piaomiandanjia) + floatval($c->jijianfei); ?>'/>
                                              </td>
                                              <td>&yen;<?php echo $c->piaomianjia; ?></td>

                                              <td>&yen;<span id='jjf_<?php echo $rt; ?>'><?php echo floatval($c->jijianfei); ?></span></td>
                                              <td><input id='chupiaobianma_<?php echo $rt; ?>' hangchengid="<?php echo $c->hangchengid; ?>" ertong ="<?php echo $myet; ?>" fancheng="<?php echo $index; ?>" onkeyup='jishuan("gmjg_<?php echo $rt; ?>",  "jjf_<?php echo $rt; ?>","chupiaobianma_<?php echo $rt; ?>", "sjtk_<?php echo $rt; ?>", "<?php echo $rt; ?>")' name='chupiaobianma_<?php echo $rt; ?>' type="text" placeholder="退票费/张"/></td>
                                              <td><span id='sjtk_<?php echo $rt; ?>'></span><input type="hidden" hangchengid="<?php echo $c->hangchengid; ?>" renshu="<?php echo $c->renshu; ?>" ertong ="<?php echo $myet; ?>" fancheng="<?php echo $index; ?>" id='refund_fee<?php echo $rt; ?>' name='refund_fee<?php echo $rt; ?>'/></td>
                                              <td><?php echo $c->chupiaobianma; ?></td>
                                              <td><button type="button" class="btn btn-warning" onclick="tuigaiqianguize('<?php echo $c->tuigaiid; ?>');">退改签规则</button></td>
                                          </tr>
                                          <?php endforeach; ?>
                                          <?php endif; ?>
                                     </tbody>
                                </table>
                                <br />
                                <table class="table mytable" style="margin-bottom:0px;">
                                     <thead>
                                          <tr>
                                              <th>乘客类型</th>
                                              <th>乘机人</th>
                                              <th>证件类型</th>
                                              <th>证件号码</th>
                                              <th>出生日期</th>
                                              <th>票号</th>
                                              <th>行程单</th>
                                          </tr>
                                     </thead>
                                     <tbody>
                                                                              <?php if (!empty($tuipiao) && !empty($tuipiao->lst) && $index == 1): ?>
									      <?php
										  foreach ($tuipiao->lst as $ck): 
                                                                                      ?>
                                          <tr>
										      <?php if ($ck->shifouertong == 0): ?>
                                                                                          <td>成人</td>
											  <?php else: ?>
											  <td>儿童</td>
											  <?php endif; ?>
                                              <td><?php echo $ck->zhongwenming;?></td>
                                              <td><?php $vs = $CI->config->item("证件类型"); echo $vs[intval($ck->zhengjianleixing)]; ?></td>
                                              <td><?php echo $ck->zhengjianhaoma; ?></td>
                                              <td><?php echo date('Y-m-d', strtotime($ck->chushengriqi)); ?></td>
                                              <td><?php echo $ck->piaohao; ?></td>
                                              <td>行程单</td>
                                          </tr>
                                          <tr>
                                              <td>退票原因：</td>
                                              <td><?php echo $ck->tuipiaoyuanyin; ?></td>
                                              <td>是否自愿：</td>
                                              <td colspan="4">
                                                  <select id="sel_tuipiaoleixing<?php echo $ck->id; ?>" name="sel_tuipiaoleixing<?php echo $ck->id; ?>" onchange="chg('<?php echo $ck->id; ?>')">
                                                      <option selected="selected" value="0">自愿</option>
                                                      <option value="1">非自愿</option>
                                                  </select>
                                                  <input type="hidden" id="tuipiaoleixing_<?php echo $ck->id; ?>" index="<?php echo $ck->id; ?>" name="tuipiaoleixing_<?php echo $ck->id; ?>" value="0" />
                                              </td>
                                          </tr>
										  <?php endforeach; ?>
                                                                                  <?php endif; ?>
                                                                              <!-- 下一个航程 -->
                                                                              <?php if (!empty($tuipiao1) && !empty($tuipiao1->lst) && $index == 2): ?>
									      <?php
										  foreach ($tuipiao1->lst as $ck): 
                                                                                      ?>
                                          <tr>
										      <?php if ($ck->shifouertong == 0): ?>
                                                                                          <td>成人</td>
											  <?php else: ?>
											  <td>儿童</td>
											  <?php endif; ?>
                                              <td><?php echo $ck->zhongwenming;?></td>
                                              <td><?php $vs = $CI->config->item("证件类型"); echo $vs[intval($ck->zhengjianleixing)]; ?></td>
                                              <td><?php echo $ck->zhengjianhaoma; ?></td>
                                              <td><?php echo date('Y-m-d', strtotime($ck->chushengriqi)); ?></td>
                                              <td><?php echo $ck->piaohao; ?></td>
                                              <td>行程单</td>
                                          </tr>
                                          <tr>
                                              <td>退票原因：</td>
                                              <td><?php echo $ck->tuipiaoyuanyin; ?></td>
                                              <td>是否自愿：</td>
                                              <td colspan="4">
                                                  <select id="sel_tuipiaoleixing<?php echo $ck->id; ?>" name="sel_tuipiaoleixing<?php echo $ck->id; ?>" onchange="chg('<?php echo $ck->id; ?>')">
                                                      <option selected="selected" value="0">自愿</option>
                                                      <option value="1">非自愿</option>
                                                  </select>
                                                  <input type="hidden" id="tuipiaoleixing_<?php echo $ck->id; ?>" index="<?php echo $ck->id; ?>" name="tuipiaoleixing_<?php echo $ck->id; ?>" value="0" />
                                              </td>
                                          </tr>
										  <?php endforeach; ?>
                                                                                  <?php endif; ?>
                                     </tbody>
                                </table>
                            </div>
                        </div>
                        <?php
                        $index++;
                        endforeach; 
						}?>
                    </div>
                </div>
                
                <div class="row">
                    <div style="float:right;">
                        <input id="mtuikuan" name="mtuikuan" type="hidden"/>
                        <input id="mfeiyong" name="mfeiyong" type="hidden"/>
                        <input id="total_fee" name="total_fee" type="hidden"/>
                        <span id="tuikuanzonge" style="color:red;font-weight:bold;">退款总额：&nbsp;&nbsp;</span>
                        <input id="wufatuipiao" name="wufatuipiao" type="hidden" value="0" />
                        <!--
                        <input type="submit" class="btn btn-success" id="btn_tuikuan" value='确定退款'></input>
                        -->
                        <input type="submit" class="btn btn-success" id="btn_tuikuan" value='确定退款'></input>
                         &nbsp;&nbsp;
                         <input type="submit" class="btn btn-danger" id="btn_wufa" value='无法退票'></input>
                    </div>
                </div>
                <input type="hidden" id='dingdanid' value='<?php echo $tp->dingdanid; ?>' name='dingdanid' />
                <input type="hidden" id='tuipiaodingdanid' value='<?php echo $tp->id; ?>' name='tuipiaodingdanid' />
                <input type="hidden" id='tuipiaoleixing' value='' name='tuipiaoleixing' />
                </form>
<?php          
$this->load->view('page/dbfooter');
?>
<script type="text/javascript">
    
    function tuipiaoleixing(tuipiaoid, leixing)
    {
        this.tuipiaoid = tuipiaoid;   
        this.leixing = leixing;
    }
    
    // 退款计算
    function mtuikaun(ertong, fancheng, refund_fee, hangchengid)
    {
        this.ertong = ertong;   
        this.fancheng = fancheng;
        this.refund_fee = refund_fee;
        this.hangchengid = hangchengid;
    }
     // 退款手续费
    function mfeiyong(ertong, refund_shouxufei, hangchengid)
    {
        this.ertong = ertong;   
        this.refund_shouxufei = refund_shouxufei;
        this.hangchengid = hangchengid;
    }
 
    function jishuan(danjia, jijianfei, zhangshu, jieguo, et) {
            $('#' + jieguo).html(parseFloat($('#' + danjia).html()) +parseFloat($('#' + jijianfei).html()) - $('#' + zhangshu).val());
            $('#refund_fee' + et).val($('#' + jieguo).html());
            
            var ze = 0;
            $("input[id^='refund_fee']").each(function(){
                ze += parseFloat($(this).val()) * parseFloat($(this).attr('renshu'));
            });
            $('#total_fee').val(ze);
            $('#tuikuanzonge').html('退款总额：'+ ze +'&nbsp;&nbsp;');
        }
        
    function chg(id)
    {
        $('#tuipiaoleixing_' + id).val($('#sel_tuipiaoleixing' + id).val());   
    }
        
    function cleartime() {
	$("#fukuanshijian_begin").val("");
        $("#fukuanshijian_end").val("");
}

function getCookie(name) {
                var arr = document.cookie.match(new RegExp("(^| )" + name + "=([^;]*)(;|$)"));
                if (arr != null)
                    return unescape(arr[2]);
                return null;
            }

        $(function () {
		$("#chupiaoform").validate({
				rules: {
                                    <?php if (!empty($hangchenglvke_tongji)): ?>
                                    <?php 
                                    $index1 = 0;
                                    $count1 = count($hangchenglvke_tongji) + count($hangchenglvke);
                                    foreach ($hangchenglvke_tongji as $c): ?>
                                       <?php if (intval($index1) != intval($count1 - 1)): ?>
                                           chupiaobianma_<?php echo $c->id; ?>:{
                                               required:true,
                                               maxlength:50
                                           },
                                       <?php else: ?>
                                           chupiaobianma_<?php echo $c->id; ?>:{
                                               required:true,
                                               maxlength:50
                                           }
                                       <?php endif; ?>
                                    <?php $index1++; endforeach; ?>
                                    <?php endif; ?>
                                        
                                    <?php if (!empty($hangchenglvke)): ?>
                                    <?php 
                                    foreach ($hangchenglvke as $c): ?>
                                       
                                       <?php if (intval($index1) != intval($count1 - 1)): ?>
                                           piaohao_<?php echo $c->id; ?>:{
                                               required:true,
                                               maxlength:50
                                           },
                                       <?php else: ?>
                                           piaohao_<?php echo $c->id; ?>:{
                                               required:true,
                                               maxlength:50
                                           }
                                       <?php endif; ?>
                                                    
                                    <?php $index1++; endforeach; ?>
                                    <?php endif; ?>
				},
				messages: {
                                      <?php if (!empty($hangchenglvke_tongji)): ?>
                                    <?php 
                                    $index1 = 0;
                                    $count1 = count($hangchenglvke_tongji) + count($hangchenglvke);
                                    foreach ($hangchenglvke_tongji as $c): ?>
                                       <?php if (intval($index1) != intval($count1 - 1)): ?>
                                           chupiaobianma_<?php echo $c->id; ?>:{
                                               required:'请填写退票编码',
                                               maxlength:'退票编码不大于 50 个字符'
                                           },
                                       <?php else: ?>
                                           chupiaobianma_<?php echo $c->id; ?>:{
                                               required:'请填写退票编码',
                                               maxlength:'退票编码不大于 50 个字符'
                                           }
                                       <?php endif; ?>
                                    <?php $index1++; endforeach; ?>
                                    <?php endif; ?>
                                        
                                    <?php if (!empty($hangchenglvke)): ?>
                                    <?php 
                                    foreach ($hangchenglvke as $c): ?>
                                       
                                       <?php if (intval($index1) != intval($count1 - 1)): ?>
                                           piaohao_<?php echo $c->id; ?>:{
                                               required:'请填写票号',
                                               maxlength:'票号不大于 50 个字符'
                                           },
                                       <?php else: ?>
                                           piaohao_<?php echo $c->id; ?>:{
                                               required:'请填写票号',
                                               maxlength:'票号不大于 50 个字符'
                                           }
                                       <?php endif; ?>
                                                    
                                    <?php $index1++; endforeach; ?>
                                    <?php endif; ?>
				}
			});
        });
        
function tuigaiqianguize(tuigaiid)
{

    var csrf_name = getCookie('csrf_cookie_name');
    $.post('<?php echo site_url("admin/guoneijipiao/tuipiao/chalvketuigaiqianBytuigaiid"); ?>/' + tuigaiid, {csrf_test_name: csrf_name}, function (data) {
      var data = $.parseJSON(data);
      $('.a_changePercentBefore').html(data['changePercentBefore']);
      $('.a_changePercentAfter').html(data['changePercentAfter']);
      $('.a_refundPercentAfter').html(data['refundPercentAfter']);
      $('.a_refundPercentBefore').html(data['refundPercentBefore']);
      $('.a_changeTimePoint').html(data['changeTimePoint']);
      $('.a_refundTimePoint').html(data['refundTimePoint']);
      $('.a_changePercentAdvance').html(data['changePercentAdvance']);
      $('.a_refundPercentAdvance').html(data['refundPercentAdvance']);
      $('.a_changeTimePointAdvance').html(data['changeTimePointAdvance']);
      $('.a_refundTimePointAadvance').html(data['refundTimePointAadvance']);
      $("#a_refund1").hide();
      $("#a_refund2").hide();
      $("#a_refund3").hide();
      $("#a_refund4").hide();
      $("#a_change1").hide();
      $("#a_change2").hide();
      $("#a_change3").hide();
      $("#a_change4").hide();

        if (data['refundPercentAdvance'] !='') {
           $("#a_refund1").show();
        } else if (data['refundTimePointAadvance'] == '' && data['refundTimePoint'] != 0 && data['refundTimePoint'] != '') {
          $("#a_refund2").show();
        } else if (data['refundTimePointAadvance'] == '' && data['refundTimePoint'] == 0 && data['refundTimePoint'] != '') {
          $("#a_refund3").show();
        } else {
          $("#a_refund4").show();
        }

        if (data['changePercentAdvance'] !='') {
           $("#a_change1").show();
        } else if (data['changeTimePointAadvance'] == '' && data['changeTimePoint'] != 0 && data['changeTimePoint'] != '') {
          $("#a_change2").show();
        } else if (data['changeTimePointAadvance'] == '' && data['changeTimePoint'] == 0 && data['changeTimePoint'] != '') {
          $("#a_change3").show();
        } else {
          $("#a_change4").show();
        }
      });
      $('#win_zhanzuo').modal('show');

}
        
        $(function () {  
		$('#chakan_a').click(function () {
                    $('#win_yidi').modal('show');
                });
                
                $('#chakan_a_bendi').click(function () {
                    $('#win_bendi').modal('show');
                });
                
                $('#btn_wufa').click(function () {
                    $('#wufatuipiao').val('1');
                });
                
                $('#btn_tuikuan').click(function () {
                    var ar = new Array();
                    $('input[id^="tuipiaoleixing_"]').each(function(index){
                        var obj = new tuipiaoleixing($(this).attr('index'), $(this).val());
                        ar.push(obj);
                    });
                    var str = JSON.stringify(ar);
                    $('#tuipiaoleixing').val(str);
                    
                var ar_cpbm = new Array();
                $("input[id^='refund_fee']").each(function (index) {
                    var tk = new mtuikaun();
                    tk.refund_fee = $.trim($(this).val());
                    tk.ertong = $(this).attr('ertong'); 
                    tk.hangchengid = $(this).attr('hangchengid');
                    tk.fancheng = $(this).attr('fancheng');
                    ar_cpbm.push(tk);
                });
                var str_cpbm = JSON.stringify(ar_cpbm);
                $('#mtuikuan').val(str_cpbm);
                
                 var ar_cpsx = new Array();
                $("input[id^='chupiaobianma']").each(function (index) {
                    var tf = new mfeiyong();
                    tf.refund_shouxufei = $.trim($(this).val());
                    tf.ertong = $(this).attr('ertong'); 
                    tf.hangchengid = $(this).attr('hangchengid');
                    ar_cpsx.push(tf);
                });
                var ar_cpsx = JSON.stringify(ar_cpsx);
                $('#mfeiyong').val(ar_cpsx);
                
                    return true;
                });
        });
        
    </script>
</body>
</html>
