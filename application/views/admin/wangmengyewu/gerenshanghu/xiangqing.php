    ...
    <div class="col-md-12">
        <div class="panel panel-primary">
            <div class="panel-heading">
                <span style="font-size:18px;">商户详情(状态: <span style="color: red;" class="shanghuzhuangtai" id="shanghuzhuangtai"><?php echo $info->zhuangtai; ?></span>)</span>
                <?php if ($info->shanghuzhuangtai==0): ?>
                    <span style="font-size:18px; float: right" ><button type="button" class="btn btn-warning" onclick="qiyong('<?php echo $info->shanghuhao; ?>')">启用账号</button></span>
                <?php endif ?>
                <?php if ($info->shanghuzhuangtai==1): ?>
                    <span style="font-size:18px; float: right" ><button type="button" class="btn btn-warning" onclick="jinyong('<?php echo $info->shanghuhao; ?>')">禁用账号</button></span>
                <?php endif ?>
            ...
            <div class="panel-body">
                ...
                <table class="table" style="margin-bottom:0px;">
                    <tbody>
                        <tr><td>注册时间：<?php echo $info->chuangjianshijian; ?></td><td>注册邮箱：<?php echo $info->email; ?></td><td></td></tr>
                        <tr><td>姓名：<?php echo $info->name; ?></td><td  colspan="2">联系人电话：<?php echo $info->shanghudianhua; ?></td></tr>
...
<!--                 <span style="font-size:18px; float: right"><a class="btn btn-warning" href='<?php echo site_url("admin/wangmengyewu/gerenshanghu/addqianyue/".$shanghuhao); ?>' role="button">签约新产品</a></span> -->
...
<script>
                        function gaizhungtai(shanghuhao, chanpinleixing) {
                            if (confirm("你确定修改吗？")) {  
                                ...
                                $.ajax({
                                    url: '<?php echo site_url("admin/wangmengyewu/gerenshanghu/gaichanpinzhungtai"); ?>',
                                    type: 'POST',
                                    dataType: 'text',
                        ...
                        function qiyong(shanghuhao) {
                            if (confirm("你确定启用吗？")) {  

                                $.ajax({
                                    url: '<?php echo site_url("admin/wangmengyewu/gerenshanghu/qiyong"); ?>',
                                    type: 'POST',
                                    dataType: 'text',
                                    data: {shanghuhao: shanghuhao,'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
                                })
                                        .done(function(data) {
                                    if (data == "true") {
                        ...
                        function jinyong(shanghuhao) {
                            if (confirm("你确定禁用吗？")) {  

                                $.ajax({
                                    url: '<?php echo site_url("admin/wangmengyewu/gerenshanghu/jinyong"); ?>',
                                    type: 'POST',
                                    dataType: 'text',
                                    data: {shanghuhao: shanghuhao,'<?php echo $this->security->get_csrf_token_name(); ?>' : '<?php echo $this->security->get_csrf_hash(); ?>'},
                                })
                        ...
                        function changezhanghu() {
                            ...
                            if (confirm("你确定修改吗？")) {  
                                $.ajax({
                                    url: '<?php echo site_url("admin/wangmengyewu/gerenshanghu/changezhanghu"); ?>',
                                    type: 'POST',
                                    dataType: 'text',
