<ion-view view-title="比比旅行">
    <ion-header-bar align-title="center" class="bar-positive" style="height:4rem">
        <!--------头部header---------->
        <div class="header f_s18 text_a_c border_b_1_d bg_fff pftop">
            <span class="heaTxx">选择收件地址</span>
            <a class="returnbut c_fff f_s18 returnjs" ng-click="selectedAddress(0,0, false)"><span></span>返回</a>
         </div>
    </ion-header-bar>
    <style type="text/css">
        .bsit_heig {
  ...
  <ion-content class="address" style="top:4rem;background: #F6F6F6;">
        ...
        <ul class="cenBox bg_fff sowjman">
            <li class="border_b_1_e9 po_re " ng-repeat="x in addressList" >
            <span ng-click="selectedAddress(x, $index, true)">
                <span class="poti_input">
                    <input type="radio" id="radio-tr-{{$index}}" name="radio-qw-v" class="regular-radio gubsit_bug " ng-checked="mail.isMail == true && $index == mail.index" /><label for="radio-tr-{{$index}}" ></label>
                </span>
