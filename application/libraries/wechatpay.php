...
class wechatpay {
	...
	public function __construct($config = array()) {
                ...
                {
                    $CI =& get_instance();
                    $wxconfig['appid'] = $CI->config->item('appid');
                    $wxconfig['mch_id'] = $CI->config->item('mch_id');
                    $wxconfig['apikey'] = $CI->config->item('apikey');
                    $wxconfig['appsecret'] = $CI->config->item('appsecret');
                    $wxconfig['sslcertPath'] = $CI->config->item('sslcertPath');
                    $wxconfig['sslkeyPath'] = $CI->config->item('sslkeyPath');
                    $this->_config = $wxconfig;
                }
                else
                {
		    $this->_config = $config;
    ...
    private function __CreateOauthUrlForCode($redirectUrl) {
        $urlObj["appid"] = $this->_config["appid"]; // $this->config->item('appid');
    ...
    public function oauth() {
        if (!isset($_GET['code'])) {
            ...
            $CI =& get_instance();
            $CI->load->library('CI_Wechat');
            $json = $CI->ci_wechat->getOauthAccessToken();
            $openid = $json['openid'];
            //注册用户，成功后可以抢单
